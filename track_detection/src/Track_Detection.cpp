...
void Track_Detection::Open_Imread(VideoCapture capture)
{
...
int main(int argc, char** argv)
{
    ...
    string test = gstreamer_pipeline(320,240,320,240,30,0);
    VideoCapture capture(test,CAP_GSTREAMER);
    if(!capture.isOpened())
    {
    ...
    while(ros::ok())
    {
        HiBotTrackDetection.Open_Imread(capture);
